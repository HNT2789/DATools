        ...
        if resp.status_code == 200:
            ...
            for item in soup.find_all("a", href=True):
                if (
                    ...
                    and item["href"].find(target) != -1
                    and item["href"].find("-site:") == -1
                ):
...
class Wordlist:
    ...
    def get(domain):
        ...
        if "local" in config_wordlist["default"]:
            local = (
                list(Wordlist.local(config_wordlist["local"]))
                if "local" in config_wordlist["default"]
                else []
            )
        ...
        if "remote" in config_wordlist["default"]:
            google = (
                list(Wordlist.google(domain))
                if "google" in config_wordlist["remote"]
                else []
            )
            duckduckgo = (
                list(Wordlist.duckduckgo(domain))
                if "duckduckgo" in config_wordlist["remote"]
                else []
            )
            virustotal = (
                list(Wordlist.virustotal(domain, config_api["virustotal"]))
                if "virustotal" in config_wordlist["remote"]
                else []
            )
...
class Output:
    ...
    def headerPrint(local, google, duckduckgo, virustotal, domain):
        ...
        line += "\n"
        line += Output.colorizeHeader(
            "Wordlist: ", local + google + duckduckgo + virustotal, "| "
        )
...
class Start:
    ...
    def parse_and_exit(args):
        if len(args) == 3 and args[1] in ["--report", "--plot", "--csv", "--set"]:
            ...
            if args[1] == "--report":
                ...
                sys.exit(
                    "use: knockpy --report path/to/domain.com_yyyy_mm_dd_hh_mm_ss.json"
                )
            ...
            if args[1] == "--plot":
                if args[2].endswith(".json"):
                    ...
                    sys.exit("report not found: %s" % args[2])
                sys.exit(
                    "use: knockpy --plot path/to/domain.com_yyyy_mm_dd_hh_mm_ss.json"
                )
            ...
            if args[1] == "--csv":
                if args[2].endswith(".json"):
                    ...
                    sys.exit("report not found: %s" % args[2])
                sys.exit(
                    "use: knockpy --csv path/to/domain.com_yyyy_mm_dd_hh_mm_ss.json"
                )
    ...
    def arguments(x=None):
        ...
        description += "-" * 80
        epilog = (
            "warning:\tapikey virustotal missing (https://www.virustotal.com/)\n\n"
            if not config["api"]["virustotal"]
            else "\n\n"
        )
        ...
        parser.add_argument(
            "-v", "--version", action="version", version="%(prog)s " + Start.__version__
        )
        parser.add_argument(
            "--no-local",
            help="local wordlist ignore",
            action="store_true",
            required=False,
        )
        parser.add_argument(
            "--no-remote",
            help="remote wordlist ignore",
            action="store_true",
            required=False,
        )
        parser.add_argument(
            "--no-http",
            help="http requests ignore\n\n",
            action="store_true",
            required=False,
        )
        parser.add_argument(
            "--no-http-code",
            help="http code list to ignore\n\n",
            nargs="+",
            dest="code",
            type=int,
            required=False,
        ...
        parser.add_argument(
            "-w", help="wordlist file to import", dest="wordlist", required=False
        )
        parser.add_argument(
            "-o",
            help="report folder to store json results",
            dest="folder",
            required=False,
        )
        parser.add_argument(
        ...
        if domain.startswith("http"):
            sys.exit("remove protocol http(s)://")
        ...
        if args.folder:
            if not os.access(args.folder, os.W_OK):
                sys.exit("folder not writable: " + args.folder)
...
def main():
    ...
    max_len = (
        len(max(wordlist, key=len) + "." + domain)
        if wordlist
        else sys.exit("\nno wordlist")
    )
    if not wordlist:
    ...
    for subdomain in wordlist:
        ...
        if https:
            ...
            Output.progressPrint(
                percentage + "dns: %s | https: no | http://%s" % (ip_req, target)
            )
